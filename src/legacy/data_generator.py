...
def enrich_dataset(
    ...
    player_data, team_data, ts_lookup = lol.trueskill_model(
        player_data, team_data, initial_sigma=2.75
    )
    ...
    flattened_teams = (
        team_data.sort_values(["teamid", "date"])
        .groupby("teamid")
        .tail(1)
    ...
    flattened_teams.to_csv(
        filepath.joinpath("data", "processed", "flattened_teams.csv"), index=False
    )

    flattened_players = (
        player_data.sort_values(["playerid", "date"])
    ...
    flattened_players[["trueskill_mu", "trueskill_sigma"]] = flattened_players.apply(
        lambda row: [ts_lookup[row.playerid].mu, ts_lookup[row.playerid].sigma],
        axis=1,
        result_type="expand",
    )
    flattened_players.to_csv(
        filepath.joinpath("data", "processed", "flattened_players.csv"), index=False
    )
...
def main():
    ...
    data = oracle.download_data(years=years)

    # Remove Buggy Matches (both red/blue team listed as same team, invalid for elo/TrueSkill)
    invalid_games = [
        "NA1/3754345055",
        "NA1/3754344502",
        "ESPORTSTMNT02/1890835",
        "NA1/3669212337",
        "NA1/3669211958",
        "ESPORTSTMNT02/1890848",
        "ESPORTSTMNT02_1932895",
        "ESPORTSTMNT02_1932914",
    ]
