...
async def schedule(ctx, league):
    try:
        ...
        output = (
            output[output["league"].str.lower() == lower_league]
            .drop(["league"], axis=1)
...
async def profile(ctx, entity):
    try:
        ...
        players = pd.read_csv(
            Path.cwd().joinpath(
                "esports-analytics", "data", "processed", "flattened_players.csv"
            )
        )
        teams = pd.read_csv(
            Path.cwd().joinpath(
                "esports-analytics", "data", "processed", "flattened_teams.csv"
            )
        )

        players_list = [
            p.lower() if isinstance(p, str) else p for p in players.playername.unique()
        ]
        teams_list = [
            t.lower() if isinstance(t, str) else t for t in teams.teamname.unique()
        ]
        lower_entity = str(entity).lower()

        if lower_entity in players_list:
            data = players[players.playername.str.lower() == lower_entity].to_dict(
                orient="records"
            )[-1]
            output = (
                f"{entity} Profile: \n \n"
            ...
            data = teams[teams.teamname.str.lower() == lower_entity].to_dict(
                orient="records"
            )[-1]
            output = (
                f"{entity} Profile: \n \n"
...
async def mock_draft(
    ...
    try:
        output = mp.mock_draft(
            blue1, blue2, blue3, blue4, blue5, red1, red2, red3, red4, red5
        )
    except Exception as e:
        output = (
            f"Something went wrong, sorry about that. \n"
...
async def validate(ctx, method, graph):
    try:
        ...
        if method.lower() == "team elo":
            ...
            raise ValueError(
                "Method must be either team elo, player elo, trueskill, side ema, egpm dom, or ensemble."
            )
