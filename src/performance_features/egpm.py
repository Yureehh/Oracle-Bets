...
def calculate_egpm_dominance_ratios(data, rank, opp_rank):
    # TODO: Should we change model to rely on instead of trueskill, using a model without volatility?
    # Or also adapting trueskill winrate calculation to use the same model? To keep into account the volatility
    data["egpm_dominance_ratio"] = data["egpm"] / (opp_rank / rank)
    data["egpm_opp_dominance_ratio"] = data["opponent_egpm"] / (rank / opp_rank)
    return data


def calculate_ema_features(data, identity, feature_name):
    # We will nans with another forward fill and then backfill just to avoid losing the first value / temporal progression
...
def calculate_dominance_metrics(data, identity):
    ...
    data["egpm_dominance_diff"] = (
        data["egpm_dominance_ratio_ema_before"]
        - data["egpm_opp_dominance_ratio_ema_before"]
    )

    return data


# * Below is the original code, that has to be moved into the predi
...
def egpm_model(
    data: pd.DataFrame, entity: str, store_model: bool = True
) -> pd.DataFrame:
    """
    ...
    data = calculate_egpm_dominance_ratios(data, rank, opp_rank)
