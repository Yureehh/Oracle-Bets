...
def compute_ema_side(df: pd.DataFrame, side: str, identity: str) -> pd.DataFrame:
    ...
    side_df[f"ema_{side.lower()}_side_before"] = side_df.groupby(identity)["result"].transform(
        lambda x: x.ewm(halflife=HALF_LIFE, ignore_na=True).mean().shift().bfill()
    )
    side_df[f"ema_{side.lower()}_side_after"] = side_df.groupby(identity)["result"].transform(
    ...
    side_df[f"ema_{side.lower()}_side_before"] = side_df.groupby(identity)[f"ema_{side.lower()}_side_before"].transform(
...
def calculate_ema_side_win_perc(row):
    ...
    if pd.notnull(row[ema_side_col]) and pd.notnull(row[ema_opp_side_col]):
        ...
        ema_side_win_perc = round(ema_side_value / (ema_side_value + ema_opp_side_value + 1e-8), 3)
...
def side_win_rate_ewm_performance(df: pd.DataFrame, entity: str) -> pd.DataFrame:
    ...
    merged = (
        pd.concat([red_side, blue_side], ignore_index=True).sort_values(get_sorting_keys(entity)).reset_index(drop=True)
    )
    ...
    for color in ["red", "blue"]:
        ...
        merged[f"ema_opp_{color}_side_before"] = oe.get_opponent(merged[f"ema_{color}_side_before"].to_list(), entity)
