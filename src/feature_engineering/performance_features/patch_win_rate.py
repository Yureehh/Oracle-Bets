...
def compute_ema_patch(df: pd.DataFrame, identity: str) -> pd.DataFrame:
    ...
    df["patch_total_games"] = grouped["gameid"].transform("size")
    df["patch_wins"] = grouped["result"].transform("sum")
    df["patch_win_rate"] = df["patch_wins"] / df["patch_total_games"]
    ...
    df["ema_patch_win_rate_before"] = grouped["ema_patch_win_rate_before"].transform(lambda x: x.bfill().ffill())

    return df


def calculate_ema_patch_win_likelihood(row: pd.Series) -> Union[float, np.nan]:
    """
    ...
    Returns:
        Union[float, np.nan]: Calculated EMA patch win likelihood, or NaN if not computable.
    """
    ema_win_rate = row["ema_patch_win_rate_before"]
    opp_ema_win_rate = row["opp_patch_ema_win_rate_before"]
...
def patch_win_rate_ewm_performance(df: pd.DataFrame, entity: str) -> pd.DataFrame:
    ...
    df["opp_patch_ema_win_rate_before"] = oe.get_opponent(df["ema_patch_win_rate_before"].to_list(), entity)

    # Calculate win likelihood based on EMA
    df["ema_patch_win_likelihood"] = df.apply(calculate_ema_patch_win_likelihood, axis=1)
