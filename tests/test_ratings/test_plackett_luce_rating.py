...
class Test_Plackett_Luce:
    ...
    def test_rate_teams_or_players_with_default_ratings(self, plackett_luce_model):
        ...
        updated_ratings = rate_teams_or_players(model, [team1_base_rating, team2_base_rating], ranks)
        ...
        assert updated_team1_rating[0].mu > team1_base_rating[0].mu, "Team 1's mu rating should increase."
        assert updated_team2_rating[0].mu < team2_base_rating[0].mu, "Team 2's mu rating should decrease."

        # Assert the sigma values have been updated (indicating a change in uncertainty)
        assert updated_team1_rating[0].sigma < team1_base_rating[0].sigma, "Team 1's sigma should be updated."
        assert updated_team2_rating[0].sigma < team2_base_rating[0].sigma, "Team 2's sigma should be updated."

    def test_calculate_plackett_luce_for_team(self, plackett_luce_df_team):
        entity = "team"

        # Calculate Plackett-Luce ratings for teams
        plackett_luce_df_team = calculate_plackett_luce(plackett_luce_df_team, entity=entity)
        ...
        assert all(
            ...
            > plackett_luce_df_team[plackett_luce_df_team["teamid"] == 1]["pl_pre_match_mu"]
        ...
        assert all(
            ...
            < plackett_luce_df_team[plackett_luce_df_team["teamid"] == 1]["pl_pre_match_sigma"]
        ...
        assert all(
            ...
            < plackett_luce_df_team[plackett_luce_df_team["teamid"] == 2]["pl_pre_match_mu"]
        ...
        assert all(
            ...
            < plackett_luce_df_team[plackett_luce_df_team["teamid"] == 2]["pl_pre_match_sigma"]
        ), "Team 2's sigma should decrease."
        # Check that team3 and team4 have been added to the ratings
        assert len(plackett_luce_df_team[plackett_luce_df_team["teamid"] == 3]) > 0, "Team 3 should be in the ratings."
        assert len(plackett_luce_df_team[plackett_luce_df_team["teamid"] == 4]) > 0, "Team 4 should be in the ratings."
        # Check that team3 mu is higher than team4 mu
        assert (
            plackett_luce_df_team[plackett_luce_df_team["teamid"] == 3]["pl_mu"].values[0]
            > plackett_luce_df_team[plackett_luce_df_team["teamid"] == 4]["pl_mu"].values[0]
    ...
    def test_calculate_plackett_luce_for_player(self, plackett_luce_df_player):
        ...
        # Calculate Plackett-Luce ratings for teams
        plackett_luce_df_player = calculate_plackett_luce(plackett_luce_df_player, entity=entity)
        ...
        assert all(
            ...
            > plackett_luce_df_player[plackett_luce_df_player["playerid"] == 1]["pl_pre_match_mu"]
        ), "Team 1's mu should increase."
        # Check every line where playerid is 1 the sigma is lower than the pre_match_sigma column
        assert all(
            plackett_luce_df_player[plackett_luce_df_player["playerid"] == 1]["pl_sigma"]
            < plackett_luce_df_player[plackett_luce_df_player["playerid"] == 1]["pl_pre_match_sigma"]
        ), "Team 1's sigma should decrease."
        ...
        assert all(
            ...
            < plackett_luce_df_player[plackett_luce_df_player["playerid"] == 2]["pl_pre_match_mu"]
        ), "Team 2's mu should decrease."
        # Check every line where playerid is 2 the sigma is lower than the pre_match_sigma column
        assert all(
            plackett_luce_df_player[plackett_luce_df_player["playerid"] == 2]["pl_sigma"]
            < plackett_luce_df_player[plackett_luce_df_player["playerid"] == 2]["pl_pre_match_sigma"]
        ), "Team 2's sigma should decrease."
        # Check that team3 and team4 have been added to the ratings
